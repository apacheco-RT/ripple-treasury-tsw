      ...
      <div className="container mx-auto px-6 max-w-6xl">
        <motion.div initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
          <h2 className="text-xl font-medium text-white mb-2 flex items-center gap-3">
            <span className="w-8 h-8 rounded-[var(--m3-shape-full)] bg-purple-500/15 border border-purple-500/25 text-purple-400 text-sm font-bold flex items-center justify-center">C</span>
          ...
          <div className="space-y-3">
            {priority.map((p, i) => (
              <motion.div
                ...
                className="flex items-center gap-4 bg-surface-card rounded-[var(--m3-shape-md)] border border-slate-800 px-5 py-4 hover:border-slate-700 transition-colors"
              >
                <div className="w-12 h-12 rounded-[var(--m3-shape-md)] bg-surface-card border border-slate-700 flex items-center justify-center shrink-0">
                ...
                <span className={`text-xs font-bold px-2.5 py-1 rounded-[var(--m3-shape-sm)] border shrink-0 ${sourceColor[p.source]}`}>
                  {p.source}
                </span>
