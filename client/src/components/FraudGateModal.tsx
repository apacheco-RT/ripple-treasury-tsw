      ...
      {open && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
          ...
          <motion.div ref={dialogRef}
            ...
            className="bg-surface-card border border-rose-500/40 rounded-(--m3-shape-xl) shadow-2xl max-w-md w-full p-6">
            <div className="flex items-start gap-3 mb-4">
              <div className="w-10 h-10 rounded-(--m3-shape-md) bg-rose-500/15 border border-rose-500/30 flex items-center justify-center shrink-0">
            ...
            <div className="bg-rose-500/8 border border-rose-500/20 rounded-(--m3-shape-md) p-3 mb-5 space-y-1.5" role="list" aria-label="High-risk transactions">
            ...
            <div className="flex flex-col gap-2">
              <button onClick={onReview} autoFocus
                className="w-full h-(--m3-button-height) rounded-(--m3-shape-full) bg-teal-600 hover:bg-teal-500 text-white font-medium text-sm transition-colors flex items-center justify-center gap-2 focus:outline-hidden focus:ring-2 focus:ring-teal-400 focus:ring-offset-2 focus:ring-offset-surface-card">
                <ShieldCheck className="w-4 h-4" aria-hidden="true" /> Review flagged payments
              </button>
              <button onClick={onApproveAnyway}
                className="w-full h-(--m3-button-height) rounded-(--m3-shape-full) bg-rose-500/15 hover:bg-rose-500/25 text-rose-300 border border-rose-500/25 font-medium text-sm transition-colors focus:outline-hidden focus:ring-2 focus:ring-rose-400 focus:ring-offset-2 focus:ring-offset-surface-card">
                Approve all anyway (not recommended)
              </button>
              <button onClick={onClose}
                className="text-slate-400 hover:text-slate-300 text-sm text-center transition-colors h-(--m3-button-height) focus:outline-hidden focus:ring-2 focus:ring-teal-400 rounded-(--m3-shape-full)">
                Cancel
              </button>
            </div>
          </motion.div>
