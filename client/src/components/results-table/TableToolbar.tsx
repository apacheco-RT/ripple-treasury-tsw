        ...
        {selected.length > 0 ? (
          <div className="flex items-center gap-2" role="toolbar" aria-label="Bulk actions">
            ...
            {handleHold && (
              <button onClick={handleHold}
                className="flex items-center gap-1.5 px-4 h-10 rounded-full text-xs font-medium bg-amber-500/15 hover:bg-amber-500/25 text-amber-300 border border-amber-500/25 transition-colors focus:outline-hidden focus:ring-2 focus:ring-amber-400 focus:ring-offset-1 focus:ring-offset-transparent">
          ...
          <span className="text-xs text-[var(--ds-color-text-secondary)] flex items-center gap-1.5">
            ...
            {sortCol === "urgency"
              ...
              : <>{`Sorted by ${sortCol}`} · <button onClick={() => setSortCol("urgency")} className="text-teal-400 hover:underline focus:outline-hidden focus:ring-2 focus:ring-teal-400 rounded-[var(--ds-radius-xs)]">Reset to urgency sort</button></>}
          </span>
        )}
      </div>
