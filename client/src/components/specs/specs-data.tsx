...
export function SeverityBadge({ level }: { level: "HIGH" | "MEDIUM" | "LOW" }) {
  ...
  const s = map[level];
  return <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-(--m3-shape-sm) text-xs font-bold border ${s.cls}`}>{s.icon}{level}</span>;
}
...
export function FeasBadge({ level }: { level: "LOW EFFORT" | "MED EFFORT" | "HIGH EFFORT" }) {
  ...
  return <span className={`inline-flex items-center px-2 py-0.5 rounded-(--m3-shape-sm) text-xs font-bold border ${cls}`}>{level}</span>;
}
...
export function PriorityBadge({ rank }: { rank: string }) {
  ...
  return (
    <div className={`w-12 h-12 rounded-(--m3-shape-md) flex items-center justify-center text-sm font-bold shrink-0
...
export function SectionDivider({ label, color }: { label: string; color: string }) {
  return (
    <div className={`flex items-center gap-3 p-(--m3-card-padding) rounded-(--m3-shape-md) border mb-6 ${color}`}>
