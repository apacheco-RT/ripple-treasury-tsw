    ...
    <section className="pt-28 pb-12 relative overflow-hidden">
      ...
      <div className="container mx-auto px-6 relative z-10 max-w-5xl">
        <motion.div initial="hidden" animate="visible" variants={{ hidden: { opacity: 0 }, visible: { opacity: 1, transition: { staggerChildren: 0.1 } } }}>
          <motion.div variants={fadeInUp} className="inline-flex items-center gap-2 px-3 py-1.5 rounded-(--m3-shape-full) bg-teal-500/10 border border-teal-500/20 text-teal-400 text-xs font-bold uppercase tracking-widest mb-4">
          ...
          <motion.div variants={fadeInUp} className="bg-slate-800/40 border border-surface-border rounded-(--m3-shape-md) p-(--m3-card-padding) max-w-4xl mb-10">
          ...
          <motion.div variants={fadeInUp} className="flex flex-wrap gap-3 text-xs">
            {legendItems.map((l, i) => (
              <div key={i} className="flex items-center gap-2 px-3 py-1.5 rounded-(--m3-shape-sm) bg-surface-card border border-slate-800 text-slate-400">
