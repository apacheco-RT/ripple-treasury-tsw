      ...
      {overrideOpen && (
        ...
        >
          <motion.div
            ...
            className="bg-surface-card border border-slate-700/60 rounded-(--m3-shape-xl) shadow-2xl w-full max-w-md mx-4 p-6"
          ...
          >
            ...
            <div className="rounded-(--m3-shape-sm) bg-amber-500/10 border border-amber-500/30 p-3 mb-4">
            ...
            <textarea
              ...
              rows={4}
              className="w-full bg-surface-page border border-slate-700/60 text-slate-200 text-sm rounded-(--m3-shape-xs) px-3 py-2 resize-none focus:outline-hidden focus:ring-2 focus:ring-teal-400 placeholder:text-slate-400 transition-all"
            />
            ...
            <div className="flex gap-3 justify-end">
              <button onClick={() => setOverrideOpen(false)}
                className="px-6 h-(--m3-button-height) rounded-(--m3-shape-full) border border-surface-border text-slate-300 hover:text-white hover:bg-(--m3-state-hover) text-sm font-medium transition-colors focus:outline-hidden focus:ring-2 focus:ring-teal-400">
                Cancel
              </button>
              <button
                disabled={!overrideReason.trim()}
                onClick={onConfirm}
                className={`px-6 h-(--m3-button-height) rounded-(--m3-shape-full) text-white text-sm font-medium transition-all focus:outline-hidden focus:ring-2 focus:ring-rose-400 disabled:opacity-40 disabled:cursor-not-allowed ${overrideReason.trim() ? "bg-rose-600 hover:bg-rose-500" : "bg-slate-700"}`}>
                Confirm Override
              </button>
            </div>
          </motion.div>
