...
2. Paste the repository URL — Replit detects `.replit` config automatically
...
4. Click **Run** to start the dev server or **Deploy → Autoscale** to publish
...
`@ds-foundation/react` is vendored at `vendor/ds-foundation-react-0.1.0.tgz` — no auth required.
...
**Action flow:** "Initiate Action" opens `ActionModal` — select accounts, choose full or partial amount, submit creates a new in-flight action. GBP payments flag a manual bank release warning (LHV has no CHAPS API).
